...
def inventory():
    ...
    if full:
        ...
        return json.dumps(
            {
                "inventory": [x.as_dict() for x in inventory_list],
                "items": [x.as_dict() for x in item_list],
            }
        )
...
def create_inventory():
    try:
        db = db_provider.DbProvider("test.db")
        req = request.get_json()
        new_item = inventory_item.InventoryItem(
            id=None, item_id=req["item_id"], quantity=req["quantity"]
        )
...
def shipments():
    ...
    if full:
        ...
        return json.dumps(
            {
                "shipments": [x.as_dict() for x in shipment_list],
                "shipment_inventory": [x.as_dict() for x in shipment_item_list],
...
def create_shipment_inventory():
    try:
        db = db_provider.DbProvider("test.db")
        req = request.get_json()
        new_item = shipment.ShipmentInventoryItem(
            id=None,
