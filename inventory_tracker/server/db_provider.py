...
class DbProvider:
    def __init__(self, db_filepath):
        ...
        cur.execute(
            """CREATE TABLE shipment_inventory_items
    ...
    def edit_item(self, item_instance):
        cur = self.con.cursor()
        cur.execute(
            f'UPDATE items SET name = "{item_instance.name}", '
    ...
    def create_inventory(self, inventory):
        ...
        db_result = cur.execute(
            f"SELECT rowid FROM inventory_items WHERE item_id = {inventory.item_id}"
        )
    ...
    def get_shipment_inventory_items(self):
        ...
        for row in cur.execute(
            "SELECT rowid, shipment_id, item_id, quantity FROM shipment_inventory_items"
        ):
    ...
    def get_full_shipment(self):
        ...
        items = self.get_items(
            [shipment_inventory_item.item_id for shipment_inventory_item in shipment_item_list]
        )
    ...
    def create_shipment_inventory(self, shipment_inventory):
        ...
        if (
            cur.execute(
                f"SELECT rowid FROM shipment_inventory_items WHERE shipment_id = "
                f"{shipment_inventory.shipment_id} AND item_id = {shipment_inventory.item_id}"
            ).fetchone()
            is not None
        ):
            raise Exception(
                f"Shipment with id = {shipment_inventory.shipment_id} and item with id = "
                f"{shipment_inventory.item_id} already exists"
            )
        db_result = cur.execute(
            f"SELECT rowid, item_id, quantity FROM inventory_items WHERE item_id = "
            f"{shipment_inventory.item_id}"
        ).fetchone()
        if db_result is None or db_result[2] < shipment_inventory.quantity:
            """Item not in stock"""
            raise Exception(
                f"{shipment_inventory.quantity} item(s) with id = "
                f"{shipment_inventory.item_id} not available"
            )
        elif db_result[2] == shipment_inventory.quantity:
            """We deplete the stock"""
            cur.execute(
                f"""INSERT INTO shipment_inventory_items VALUES
                           ({shipment_inventory.shipment_id},
                            {shipment_inventory.item_id},
                            {shipment_inventory.quantity})"""
            )
